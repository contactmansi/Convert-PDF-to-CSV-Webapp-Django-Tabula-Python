...
from .errors import UnexpectedFile, InvalidInput, StorageError
from .business_use_cases import Business_use_cases
import csv
import os
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Catch all Exceptions raised from tasks.py while running use cases
                ...
                # For Any other exception display the exception message alongwith User Input form seeking inputs str(e).split("]", 1)[1]
                return render(request, 'dehaat_app/input.html', {'form': form, 'error': str(e) + ': Please provide valid inputs again'})
