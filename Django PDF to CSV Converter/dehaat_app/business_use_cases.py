...
from .errors import UnexpectedFile, InvalidInput
...
class Business_use_cases:

    def __init__(self, user_input_file, input_variable, input_year):
    ...
    def run_use_cases(self):
        self.output_file_path = Business_use_cases.PdfConverter(self.user_input_file)
        Business_use_cases.delete_Transactions()
        self.check_storage_success = Business_use_cases.parse_csv(self.output_file_path)
        if self.check_storage_success == True:
            self.output_amount = Business_use_cases.retrieve_Amount(
                self.input_variable, self.input_year)
        else:
            raise StorageError
        self.output_dict = {'output_file_path': self.output_file_path,
    ...
    def PdfConverter(user_input_file):
        ...
        df = tabula.read_pdf(os.path.join(
            files_directory, user_input_file.name), stream=True, pages=1)
        ...
        try:
            ...
            Extracted_df.iloc[:, 2][0:6] = Extracted_df.iloc[:,
                                                             2][0:6].apply(lambda text: text.split(" ", 1)[0])
            Extracted_df.iloc[:, 3] = Joined_column[0:6].apply(
                lambda text: text.split(" ", 1)[1] if len(text.split(" ", 1)) > 1 else "")
    ...
    def parse_csv(output_file_path):
        ...
        for (variable, amount_year1, amount_year2) in zip(output_df[0], output_df[1], output_df[2]):
            ...
            if(str(variable) != 'nan' and variable != variable_col_name):
                transaction1 = Transaction(particulars=variable.lower(),
                                           amount=amount_year1,  year=year1)
                transaction2 = Transaction(particulars=variable.lower(),
                                           amount=amount_year2,  year=year2)
                transaction1.save()
                transaction2.save()
    ...
    def retrieve_Amount(input_variable, input_year):
        ...
        try:
            output_transaction_list = Transaction.objects.filter(
                particulars=input_variable, year=input_year)
